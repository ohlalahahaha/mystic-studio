...
function webShot(c, url, out, opts = {}) {
  ...
  if (opts.dpr) args.push('--dpr', String(opts.dpr));
...
// ---------- doctor ----------
// Live truth about the Gemini key. A key's shape proves nothing (2026+ keys start
// "AQ.", not "AIza") — only the API's own answer counts. Verified 2026-09-27:
// a healthy key returns 200, a dead key 401/403, an empty prepay balance 402.
function geminiProbe(c) {
  if (!c.geminiKey) return 'put GEMINI_API_KEY in the env file (copy .env.example)';
  if (!cfg.firstOnPath('curl')) return 'set (live check needs curl — install it to verify)';
  const base = 'https://generativelanguage.googleapis.com/v1beta';
  const code = (args) => {
    try {
      const r = sh('curl', ['-s', '-o', '/dev/null', '-w', '%{http_code}', '--max-time', '10', ...args], 15000);
      return (r.stdout || '').trim();
    } catch { return '000'; }
  };
  const auth = code(['-H', `x-goog-api-key: ${c.geminiKey}`, `${base}/models`]);
  if (auth === '000') return 'set (network unreachable — could not verify live)';
  if (auth === '401' || auth === '403') return 'set but REJECTED (401/403) — rotate the key at aistudio.google.com/apikeys';
  if (auth !== '200') return `set (unexpected ${auth} listing models — check the key)`;
  const gen = code(['-X', 'POST', '-H', `x-goog-api-key: ${c.geminiKey}`, '-H', 'Content-Type: application/json',
    '-d', '{"contents":[{"parts":[{"text":"ping"}]}],"generationConfig":{"maxOutputTokens":1}}',
    `${base}/models/gemini-flash-latest:generateContent`]);
  if (gen === '200') return 'set and VERIFIED live (key valid, billing working)';
  if (gen === '402') return 'key valid but PREPAY DEPLETED (402) — buy credits at ai.studio/projects (billing). Key shape is fine; do not rotate it.';
  if (gen === '429') return 'key valid but rate-limited right now (429) — retry later';
  if (gen === '404') return 'key valid (auth passed); probe model retired — spot-check a vision call';
  return `key valid (models list 200); generation returned ${gen}`;
}
...
function doctor(c) {
  ...
  ok('gemini key', !!c.geminiKey, geminiProbe(c));
    ...
    case 'web_shot': {
      ...
      if (a.health) {
        const rows = widths.map((w) => {
          const s = webShotHealth(c, a.url, path.join(c.outDir, `${tag}-${w}.png`), { width: w, full: a.full_page, dpr: a.dpr });
          return `${s.out} — ${formatHealth(s.health).replace(/\s+/g, ' ')}`;
        });
        return `saved:\n${rows.join('\n')}`;
      }
      const outs = widths.map((w) => webShot(c, a.url, path.join(c.outDir, `${tag}-${w}.png`), { width: w, full: a.full_page, dpr: a.dpr }));
      return `saved:\n${outs.join('\n')}`;
    }
